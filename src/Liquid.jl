...
mutable struct Liquid <: object
	...
	function Liquid()
		...
		this.Info = function()
			...
			info *= "Distribución de diámetros auxiliares: "*string(this.σ₂)*"\n"
			info *= "Distribución de energias auxiliares: "*string(this.ϵ)*"\n"
			info *= "Esfera suave: "*string(this.soft)
			println(info)
		end
...
`σ::Real` Diámetro de las partículas del sistema
Función que devuelve un objeto tipo `Liquid`.
"""
function Liquid(ϕ::Real, σ::Real)
	L = Liquid()
	L.setDistribution([ϕ], [σ], phi = true)
	return L
end

Liquid(ϕ::Real) = Liquid(ϕ::Real, 1.0)
